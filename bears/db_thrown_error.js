const db_thrown_error = err => {
  ...
  switch (err.type) {
    ...
    case `POST`:
      if (err.error !== undefined) {
        return {
          errorMessage: `Bear not created!. Internal server error. ${
            err.error
          }`,
          status: 500,
        };
    ...
    case `DELETE`:
      if (err.error !== undefined && typeof err.error === `object`) {
        if (err.error.name === 'CastError') {
          ...
          return {
            errorMessage: `Bear could not be deleted. Internal server error. ${
              err.error
            }`,
            status: 500,
          };
    ...
    case 'PUT':
      if (err.error !== undefined && typeof err.error === `object`) {
        if (err.error.name === `CastError`) {
          ...
          return {
            errorMessage: `Bear not updated. Internal server error. ${
              err.error
            }`,
            status: 500,
          };
